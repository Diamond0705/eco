...
## Реализация в фазе 12

Фаза 12 включает модель `v2` для новых расчетов. Старые `RouteOption` остаются снимками:
они получают значение по умолчанию `calculation_model_version = "v1"` и пустой
`calculation_details_json`, но не пересчитываются.

Модель `v2` использует только нормализованные данные из `route_facts_json`:

- среднюю скорость маршрута;
- `road_class_summary`;
- `surface_summary`;
- `traffic_delay_minutes`, если он когда-либо будет заполнен провайдером;
- `has_tolls` и `toll_cost_rub`, если стоимость известна.

Для GraphHopper итоговый `fuel_multiplier` строится из факторов скорости, класса дороги,
покрытия и пробок. Для mock-маршрутов сохраняются прежние демонстрационные множители
`1.08`, `1.00` и `0.92`, чтобы не применять учебный коэффициент дважды.

Стоимость `v2` включает:

```text
cost_rub =
    fuel_cost_rub
    + distance_service_cost_rub
    + time_cost_rub
    + toll_cost_rub
```

`time_cost_rub` считается по новому параметру `driver_time_tariff_rub_per_hour` в
`EcoCalculationSettings`. Значение по умолчанию для демо-данных: `900.00 руб/ч`.

Если `has_tolls = true`, но `toll_cost_rub` равен нулю или неизвестен, стоимость платной дороги
не добавляется, а в `calculation_details_json.warnings` сохраняется предупреждение:
`Платные участки обнаружены, но стоимость проезда не рассчитана.`

CO2 остается топливной оценкой в кг/литр. NOx и PM остаются упрощенной оценкой через работу
двигателя и экологический стандарт транспорта. Модель `v2` не является сертифицированной
методикой EN 16258 / EMEP / EEA / COPERT.

## Реализация в фазе 13: v2.1

Фаза 13 включает модель `v2.1` для новых расчетов. Старые `RouteOption` не пересчитываются.
Модель сохраняет абсолютные `co2_kg`, `nox_g`, `pm_g`, но эко-рейтинг строит как учебный
сравнительный показатель на основе интенсивности выбросов и небольших маршрутных штрафов.

В `calculation_details_json` для новых расчетов сохраняются:

- `co2_kg_per_km`, `nox_g_per_km`, `pm_g_per_km`;
- `co2_kg_per_ton_km`, `nox_g_per_ton_km`, `pm_g_per_ton_km`, если вес груза больше нуля;
- `emissions_score`, `route_risk_penalty`, `eco_rating_method`;
- расчетные множители и deduplicated warnings для отображения в сравнении маршрутов.

Новая область поддержки задается `MAX_ROUTE_DISTANCE_KM=2000`. Кандидаты выше лимита
фильтруются до удаления старых невыбранных вариантов; если валидных кандидатов не осталось,
расчет отклоняется русским пользовательским сообщением.

Предупреждения v2.1 объясняют ограничения данных: отсутствие live-пробок у провайдера,
платные участки без рассчитанной стоимости и значимую неизвестную долю ограничений скорости.
Эта модель остается приближенной учебной оценкой и не является сертифицированной методикой
EMEP/EEA/COPERT.

## Реализация в фазе 14: отчеты

Фаза 14 использует уже сохраненные `RouteOption` и `calculation_details_json` в путевых листах,
отчетах по выбросам и аналитике. Формулы, провайдеры и старые снимки маршрутов не меняются.

Отчеты показывают только компактные объяснимые показатели: версию модели, итоговый множитель,
среднюю скорость, интенсивность CO2, средний эко-рейтинг, платные участки и Евро-класс транспорта.
